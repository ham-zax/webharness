...
The `browser-jev` logical server exposes `jev_run`, `jev_start`, `jev_tick`, `jev_state`, and `jev_stop`. `jev_run` is the fast one-call path: it starts a run, advances the existing Jev loop internally until a terminal state, applies the same deterministic success checks after every cycle, and cleans the owned target/helper before returning the final sanitized state. The stateful start/tick/state/stop tools remain available for interactive inspection and steering. It runs the Git-pinned upstream Jev Ultrafast loop through one uv-locked Python worker per run, using TypeSafe for operation/target decisions and the configured small text model only for `TYPE_TEXT`. It resolves the same Windows Chrome and Linux browser-fast identities at each start. Live Clearcote profiles are attached through their ephemeral loopback endpoint without launching a second browser; Linux Chrome obtains the resolved session endpoint through Agent Browser `get cdp-url`. Each run owns a background target and a unique namespaced Browser Harness daemon, and stop closes only those owned resources. Profile operations serialize, tool output is sanitized, and Jev `DONE` is accepted only after all required declarative success checks pass. Tool schemas accept neither model credentials, selectors, nor executable verification logic.
