...
- Unfamiliar repository behavior, ownership, configuration, related implementation, or hybrid lexical/semantic retrieval in an indexed codebase -> Local `server="satori"`. Use `list_codebases` when readiness/root is unknown; prefer `search_codebase` for discovery and follow its returned `recommendedNextAction` into `read_file`, `file_outline`, `call_graph`, or `continue_search` only when applicable. Treat Satori graph/navigation as bounded advisory evidence, not authoritative proof that no caller/reference exists.
- Repository knowledge-graph, architecture, dependency/boundary, multi-hop relationship, trace-path, or graph-oriented impact questions in an indexed project -> Local `server="codebase-memory-mcp"`. Use `list_projects`/`index_status` when project readiness is unknown, then select focused read-oriented graph tools such as `search_graph`, `query_graph`, `trace_path`, `get_architecture`, `search_code`, or `get_code_snippet` as the question requires.
- Satori and Codebase Memory are retrieval/navigation aids, not substitutes for authoritative live source. Verify consequential edit locations and current file contents with Dev/Code before mutation. Do not invoke Satori `manage_index` create/reindex/clear or Codebase Memory index/project/ADR/trace mutations merely to make retrieval convenient; use those mutating operations only when the active task actually authorizes changing index or graph state.
