...
Linux `browser-fast` uses `~/.config/mcp-dev-bridge/browser-fast.json` for its default and managed Clearcote profile catalog; start from `config/browser-fast.example.json`. The maintained workstation keeps `clearcote` / `x-main` as the normal default. Use `browser_backend="chrome"` or `browser_backend="clearcote"` per call instead of rewriting the shared selector; use `browser_profile` only when selecting a specific managed Clearcote profile. The provider owns the persistent Clearcote profile/process, Agent Browser supplies snapshots/refs/target IDs over its ephemeral loopback CDP endpoint, and supported input actions execute through Clearcote humanization. Chrome uses its own Agent Browser session and does not require closing Clearcote. The V1 external `cdpPort` form remains readable only as a migration compatibility path. The Windows target remains the dedicated shared MCP Chrome process, and Firefox is not supported by the current Chromium-CDP driver.
