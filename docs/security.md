...
Browser-jev accepts neither credentials, selectors, nor executable verification logic through MCP. Its model settings come only from the server environment or the bounded current-user-owned file named by `MCP_BROWSER_JEV_ENV_FILE`; the renderer stores only that path in the private generated config, and the provider revalidates the file for each run. Returned state drops model request bodies, raw response heads, screenshots, browser objects, endpoints, and environment data. Each run uses a unique daemon namespace and an owned background target. Stop closes those owned resources without terminating or taking over the shared browser, while same-profile queueing prevents overlapping Jev mutations.
