...
- Local Browser access can control the dedicated persistent Windows MCP Chrome profile or the configured WSLg browser identity (Chrome or Clearcote) after explicit `tag:local` authorization; this includes browser-jev's model-driven website mutations. Everyday Windows Chrome remains outside MCP control.
...
The outer `local` provider is the `tag:local` authorization boundary. Ordinary `tool_call(server, tool, arguments)` and same-route `tool_batch(server, tool, calls)` operate on public Local downstream servers including `code`, `terminal`, `host`, Browser, and owner-added MCPs. `fallback_dispatch(server, tool, arguments)` is intentionally broader only in one direction: it can also reach the fallback-only mirror of outer Dev so an already-authorized writable Dev operation can still be attempted when its normal model-facing call is unavailable or unreliable. The fallback tool is intentionally advertised with `readOnlyHint` for transport compatibility, but that hint is not an authorization or side-effect guarantee; the selected downstream operation retains its real authority and may edit, create, delete, execute, control a PTY, sleep the host, or otherwise mutate state. Batch concurrency changes orchestration, not authorization. The `browser-devtools`, `browser-fast`, and `browser-jev` logical servers intentionally share this local browser trust domain, and owner-configured `MCP_LOCAL_SERVERS_FILE` entries join that same authority domain. The owner file is accepted only for the Personal profile, must be a bounded current-user-owned regular file, cannot replace built-in Local servers, and may launch only an existing absolute executable with literal args/env and an optional existing absolute working directory. Owner stdio servers are supervised by 1MCP on unexpected exit unless explicitly opted out. Local privately enables only 1MCP's reload action for targeted backend recovery; the reserved `1mcp` namespace is filtered from discovery and rejected as a model route, so this lifecycle authority does not become another model-facing Local metatool or a general management surface. Adding a server is therefore an explicit capability grant; a genuinely different trust domain needs a separate broker/scope or direct exposure.

Browser-jev accepts neither credentials, DOM selectors, regular expressions, nor executable verification logic through MCP. Its optional collection contract is deliberately narrower: callers may provide only a bounded visible-text start-line prefix, exact end-line marker, and numeric count/stability limits, and must pair collection with at least one URL/title/text success check that identifies the intended document. Every contributing text node must pass Chrome's rendered-visibility check, the provider retains at most 200 records, and each retained record is truncated before being returned. Its model settings come only from the server environment or the bounded current-user-owned file named by `MCP_BROWSER_JEV_ENV_FILE`; the renderer stores only that path in the private generated config, and the provider revalidates the file for each run. Returned state drops model request bodies, raw response heads, screenshots, browser objects, endpoints, and environment data. Each run uses a unique daemon namespace and an owned background target. Stop closes those owned resources without terminating or taking over the shared browser; on WSL, the worker may terminate a lingering namespaced daemon process only after a clean daemon shutdown acknowledgement and direct confirmation that its owned browser target is gone, using a `pidfd` so PID reuse cannot redirect the signal. If either target cleanup or daemon exit cannot be proven, cleanup fails closed and preserves the ownership records for later recovery. To recover from worker/provider death before that path runs, Browser-Jev stores a mode-0600 current-user lease beneath XDG state containing only non-secret ownership metadata (profile key, loopback endpoint, worker PID/start identity, helper-daemon PID/start identity, daemon namespace, and target IDs). The content tab carries only the opaque run namespace in per-tab session storage, and the helper tab carries the same opaque namespace in its `about:blank` fragment. A later same-profile run reclaims a lease only after the worker process identity is proven dead; an empty marker scan is inconclusive, same-endpoint recorded target IDs are independently confirmed absent, and active or unverifiable owners are fail-closed and never reclaimed. Same-profile queueing prevents overlapping Jev mutations.
