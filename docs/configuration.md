...
For personal rendering, the outer config contains `agents`, `code`, `dev`, `local`, and `terminal`. The outer `local` provider is tagged only `local` and points at the repository Local broker. `agents` is tagged only `agents` and points at `providers/agents/server.mjs` with the Agent Broker Unix socket in `MCP_AGENT_SOCKET`. The renderer also atomically materializes a Local inner config at the bridge state root containing `browser-devtools` and `browser-fast`, and writes that inner config before publishing the outer config so config reload cannot start Local against a missing file. Agents remains outside that inner broker so `tag:agents` is an independent client authorization domain.
...
The Agents browser adapter source is `webharness-agents-extension/`; Personal setup copies it to the stable unpacked path `~/.local/share/webharness/agents-extension`. This path and the dedicated Agents Chrome profile are separate from Browser Fast and Clearcote configuration.
