...
For personal rendering, the outer config contains `local`, `code`, `dev`, and `terminal`. The outer `local` provider is tagged only `local` and points at the repository Local broker. The renderer atomically materializes a Local inner config at the bridge state root containing the built-in `browser-devtools` and `browser-fast` surfaces, any explicitly configured `MCP_LOCAL_SERVERS_FILE` entries, and fallback-only mirrors of the fully rendered outer Dev and Terminal providers. The mirrors inherit the same environment and paths but drop outer tags. `MCP_LOCAL_FALLBACK_ONLY_SERVERS=dev,terminal` keeps those mirrors out of `tool_list`, `tool_schema`, ordinary `tool_call`, and `tool_batch`; only `fallback_dispatch` can target them. The inner config is written before publishing the outer config so config reload cannot start Local against a missing file, and the outer Local provider receives a deterministic revision of the rendered inner config so a real downstream configuration change restarts only Local through 1MCP hot reload rather than requiring a full WebHarness restart. The broker keeps 1MCP's reload management action private for targeted recovery and never exposes the reserved `1mcp` namespace through Local.
