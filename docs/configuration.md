...
`MCP_LOCAL_SERVERS_FILE` is an optional personal-profile reference to a current-user-owned JSON file outside the repository. It accepts a standard `{"version":"1.0.0","mcpServers":{...}}` object and merges those servers into the private Local inner 1MCP beside the built-in browser surfaces. Extra servers are explicit owner grants: names cannot replace built-ins, each server is limited to optional `type: "stdio"`, `command`, optional string `args`, optional absolute `cwd`, and optional string-valued `env`; `command` must be an existing absolute executable path and `cwd`, when set, must be an existing directory. The source file is capped at 128 KiB; the rendered Local inner config remains private mode `0600`. Values under `env` are copied into that private rendered config, so keep credentials there only when a downstream MCP actually requires them.

Linux `browser-fast` reads the current-user-owned `~/.config/mcp-dev-bridge/browser-fast.json` as its default/backend-profile policy. Missing configuration preserves managed Chrome. V2 can define named managed Clearcote profiles; the maintained workstation uses `x-main` as its normal Linux default. Individual `observe`/`execute` calls may set `browser_backend="chrome"` or `browser_backend="clearcote"` without rewriting the shared selector, and may set `browser_profile` for a named managed Clearcote profile. The returned Linux state identifies the resolved backend/profile; use the same values with refs and tab IDs from that observation. The older V1 `cdpPort` form is retained only as a migration compatibility path. Firefox is not supported by the current Chromium-CDP driver and fails explicitly rather than falling through to Chrome.
