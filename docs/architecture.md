...
The Local broker owns stable logical `{server, tool}` routing and connects over stdio to an inner 1MCP running in normal direct mode. It keeps no broker catalog/schema cache. Unscoped `tool_list` is server-oriented: it returns the configured logical server names with current tool availability/counts, so a large downstream catalog cannot drown out another server during discovery. `tool_list(server=...)` then enumerates only that server's tools, and `query` is scoped to server names when `server` is omitted or to the selected server's tool metadata when it is present. Schema lookup consults current inner `tools/list`, `tool_call` dispatches one qualified inner tool and returns its downstream `CallToolResult` unchanged, and `tool_batch` states one route once then dispatches a bounded set of structured argument objects with bounded concurrency. Batch routing fields and argument-object shapes are preflighted before dispatch; downstream MCPs retain ownership of their own tool-schema validation. Member results preserve input order and distinguish broker/transport rejection from a fulfilled downstream result whose own `isError` may be true. Discovery is bounded with an opaque self-contained cursor; downstream catalog churn does not change the outer four-tool surface.

The private inner 1MCP enables only its internal reload management action for broker-owned recovery; the reserved `1mcp` namespace remains rejected and filtered from the model-facing Local catalog. If a configured server is absent during scoped discovery/schema/batch preflight, Local performs one coalesced targeted reload of that server and verifies that tools reappear. A failed direct `tool_call` is never automatically replayed: if Local proves the selected server disappeared, it may recover the backend but returns a retry-required error because the original action's side-effect outcome could be ambiguous. Owner-configured stdio MCPs are rendered with 1MCP `restartOnExit: true` by default, with an explicit `false` opt-out, so ordinary post-start crashes are handled by the native supervisor before broker recovery is needed.
