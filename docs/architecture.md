...
`browser-jev` exposes exactly `jev_run`, `jev_start`, `jev_tick`, `jev_state`, and `jev_stop`. `jev_run` keeps the control loop inside one MCP request: Node starts one run, repeatedly invokes the same worker `tick` operation until a terminal state, applies the same deterministic success checks after every cycle, then cleans the owned target/helper before returning the final sanitized state. The stateful start/tick/state/stop path remains available for interactive control. Node owns the MCP contract, run registry, profile queues, endpoint resolution, sanitization, and deterministic success checks; one persistent Python worker per run imports the uv-locked upstream Jev implementation. Every start resolves the selected managed backend at call time. Managed Clearcote is live-only: browser-jev reads that profile's ephemeral `DevToolsActivePort` endpoint and instructs the operator to initialize it through browser-fast when inactive; it never launches or repairs the shared Clearcote runtime. Linux Chrome uses Agent Browser's read-only `get cdp-url` command against the resolved browser-fast session, while Windows uses the shared managed Chrome runtime.

Each run sets a unique `BU_NAME=jev-<run-id>`. Jev creates its own background CDP target and focus emulation, so it does not take over browser-fast's active mediated tab. Operations serialize by resolved target/backend/profile, while different profiles may proceed concurrently. A tick performs one TypeSafe operation/target decision and consumes only the selected operation's target; `TYPE_TEXT` remains the upstream small-text-model path. No selector or generated executable code crosses the tool contract. Jev `DONE` becomes provider `done` only after every declared URL/title/text/history check passes; failed verification becomes terminal `blocked`, and no mutation is replayed after transport ambiguity. `jev_stop` closes the owned target, stops only the run's namespaced Browser Harness daemon, and terminates the worker without closing the shared browser process.
