...
`browser-jev` exposes exactly `jev_start`, `jev_tick`, `jev_state`, and `jev_stop`. Node owns the MCP contract, run registry, profile queues, endpoint resolution, sanitization, and deterministic success checks; one persistent Python worker per run imports the uv-locked upstream Jev implementation. Every start resolves the selected managed backend at call time. Managed Clearcote is live-only: browser-jev reads that profile's ephemeral `DevToolsActivePort` endpoint and instructs the operator to initialize it through browser-fast when inactive; it never launches or repairs the shared Clearcote runtime. Linux Chrome uses Agent Browser's read-only `get cdp-url` command against the resolved browser-fast session, while Windows uses the shared managed Chrome runtime.
