...
Local
  ...
  `-- server="browser-jev"  -> stateful Jev agent loop
...
`browser-jev` exposes exactly `jev_run`, `jev_start`, `jev_tick`, `jev_state`, and `jev_stop`. `jev_run` keeps the control loop inside one MCP request: Node starts one run, repeatedly invokes the same worker `tick` operation while deterministic success remains unmet, and ends immediately when those checks pass rather than requiring a final model `DONE`. During autonomous runs a premature `DONE` is reset inside the worker so the verifier, not model belief, remains authoritative. Optional bounded text-record collection lives in the Node run manager, so evidence from virtualized pages survives after earlier DOM content scrolls away; collection requests must declare at least one URL/title/text success check that identifies the intended document; records are accepted only while those identity checks pass, and a document change resets accumulated progress. Collection progress is fed back to the worker as trusted runtime context and collection presence automatically adds deterministic `collection_complete` verification after the configured minimum plus the configured number of provider-owned no-new-record WAIT observations; a no-new scroll first schedules settling rather than counting as stable. Whole-document scroll exhaustion remains separately observable and can be required explicitly with `success.scroll_exhausted`, rather than being trusted as collection completion on a lazy-loaded page. The stateful start/tick/state/stop path remains available for interactive control. Node owns the MCP contract, run registry, collection state, profile queues, endpoint resolution, sanitization, and deterministic success checks; one persistent Python worker per run imports the uv-locked upstream Jev implementation. A provider-local snapshot script keeps at most 250 model-visible actions and selectable guards while hashing the full observed action semantics for compact freshness, and a thin managed Agent subclass adds sparse-page hydration, bounded settling for model-selected collection scrolls, transient-blocked progress recovery while deterministic work remains, plus bounded lazy-load recovery around the upstream three-scroll no-progress guard. Collection-specific startup settling is omitted when an unmet `url_contains` identity check already proves the input document is only a navigation source rather than the collection target. As soon as the configured URL/title/text identity checks identify the intended page, Node advances collection through an internal deterministic scroll/wait command—including seeking the first matching record—rather than spending another TypeSafe decision on mechanical traversal. The worker projects upstream state onto a bounded Browser-Jev wire snapshot so cumulative model request/answer internals never cross the JSON-lines boundary. Autonomous calls also observe the MCP request abort signal between bounded worker cycles and enter the same owned-resource cleanup path on cancellation. Every start resolves the selected managed backend at call time. Managed Clearcote is live-only: browser-jev reads that profile's ephemeral `DevToolsActivePort` endpoint and instructs the operator to initialize it through browser-fast when inactive; it never launches or repairs the shared Clearcote runtime. Linux Chrome uses Agent Browser's read-only `get cdp-url` command against the resolved browser-fast session, while Windows uses the shared managed Chrome runtime.

Each run sets a unique `BU_NAME=jev-<run-id>`. Jev creates its own background CDP target and focus emulation, so it does not take over browser-fast's active mediated tab. Operations serialize by resolved target/backend/profile, while different profiles may proceed concurrently. A tick first applies code-owned deterministic navigation when unmet `url_contains` checks identify one unique observed link destination: relative hrefs are normalized, a short bounded dynamic-page settle may expose the link, and execution still uses Jev's observed-node/freshness checks. Only unresolved navigation reaches the TypeSafe operation/target decision; those fallback link criteria retain the observed href alongside the semantic label so System One can distinguish otherwise-similar targets. `TYPE_TEXT` remains the upstream small-text-model path. No selector or generated executable code crosses the tool contract. Jev `DONE` becomes provider `done` only after every declared URL/title/text/history check passes; failed verification becomes terminal `blocked`, and no mutation is replayed after transport ambiguity. `jev_stop` closes the owned target, stops only the run's namespaced Browser Harness daemon, terminates the worker, and removes its ownership lease without closing the shared browser process. Each live worker keeps a mode-0600 lease under the current user's XDG state directory with its profile key, worker PID/start identity, helper-daemon PID/start identity, daemon namespace, and owned target IDs. The content browsing context also carries an opaque run marker in per-tab session storage while the daemon helper uses an `about:blank#...` marker. A later run for the same profile may reclaim a lease only when the recorded worker PID/start-time identity is dead; marker-based reclamation survives browser session restore when CDP target IDs change, while active concurrent runs are left untouched.
