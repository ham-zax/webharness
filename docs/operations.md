...
For the Personal Workstation, outer 1MCP exposes direct Dev plus Local `tool_list`, `tool_schema`, `tool_call`, `fallback_dispatch`, and `tool_batch` under `tag:local`. Local starts an inner 1MCP over stdio in normal direct mode. Unscoped `tool_list` reports public logical servers including `code`, `terminal`, `host`, `browser-devtools`, `browser-fast`, `browser-jev`, and owner-configured MCPs; the Dev mirror stays hidden there and remains rejected by ordinary `tool_call` and `tool_batch`. When the outer Dev catalog/schema path itself is unavailable, explicit read-only `tool_list(server="dev")` and `tool_schema(server="dev", tool=...)` provide recovery inspection. `fallback_dispatch` can then target that Dev mirror when the normal writable Dev call is unavailable or unreliable, while preserving the same downstream arguments and result. Its `readOnlyHint` is intentional transport metadata, not a side-effect guarantee. `tool_batch` repeats one selected public downstream route over several structured argument objects with bounded concurrency. Owner stdio MCPs are supervised with `restartOnExit: true` by default. The Local child enables only 1MCP's reload management action for broker-owned targeted recovery, while the reserved `1mcp` namespace remains filtered and unreachable through model-facing Local routing. A failed direct call is never auto-replayed after recovery because its original outcome may be ambiguous. The renderer writes the inner config before the outer config and carries a deterministic inner-config revision so downstream changes recycle only Local. Stock lazy `tool_invoke` remains outside this path because direct mode preserves downstream MCP results.

Windows browser calls do not attach to the everyday Chrome profile and require no `chrome://inspect` setup. Profileless calls use `%LOCALAPPDATA%\\mcp-dev-bridge\\chrome-profile`, which remains shared by `browser-fast` and `browser-devtools`; browser-jev creates an owned background target within the selected managed runtime. An explicit Browser Fast or Browser Jev `browser_profile=<name>` instead uses `%LOCALAPPDATA%\\mcp-dev-bridge\\chrome-profiles\\<name>` with its own Chrome process, DevTools endpoint, Agent Browser session, and operation queue; Browser DevTools currently targets only the shared default. On first use or after the selected browser exits, the runtime launches visible Chrome with `--user-data-dir=<selected directory>` and `--remote-debugging-port=0`, waits for that directory's `DevToolsActivePort`, and health-checks the loopback endpoint. Chrome chooses the port, so the bridge does not reserve a global `9222`. Profiles are persistent: sign into the selected MCP Chrome window once when needed, and cookies/local storage remain across restarts. Do not copy the everyday Chrome data directory into them. Agent Browser 0.35.0 plus its one-shot Windows Node helper remain materialized under `%LOCALAPPDATA%\\mcp-dev-bridge\\agent-browser\\0.35.0`; the helper owns bounded stdout/stderr files so cold daemon startup cannot keep the WSL interop lifetime open. Do not publish debugging endpoints beyond the trusted local machine. `browser_target=linux` selects the separate WSLg paths.
...
Use the same selector on `browser-devtools` or `browser-jev`. They attach to the live Clearcote CDP endpoint, so all surfaces see the same cookies and authenticated session while Jev owns a separate background target. If that managed profile is not running, initialize it once through `browser-fast` and retry; neither browser-devtools nor browser-jev launches a second process against the Clearcote profile.

For a Chrome operation, use `browser_target="linux"` with `browser_backend="chrome"`. On `browser-fast`, omit `browser_profile` for the existing shared Chrome behavior, or provide a stable name to create/reuse a persistent isolated profile beneath the bridge state directory. On `browser-devtools` and `browser-jev`, an explicit Chrome backend keeps the standalone Linux Chrome path; a named profile uses the corresponding persistent bridge-state directory. Browser-jev obtains the endpoint with Agent Browser's read-only `get cdp-url` command and never caches it for a later run. This does not change `browser-fast.json` or close a managed Clearcote runtime.
