...
For the Personal Workstation Local domain, the outer 1MCP exposes only Local `tool_list`, `tool_schema`, `tool_call`, and `tool_batch` under `tag:local`; the Local provider starts an inner 1MCP over stdio in normal direct mode. `tool_batch` repeats one selected downstream route over several structured argument objects with bounded concurrency, avoiding shell/CLI orchestration. That inner config contains the `browser-devtools` and `browser-fast` surfaces. The renderer writes the inner config before the outer config so a hot reload cannot start Local against a missing file. Stock lazy `tool_invoke` remains outside this path because direct mode preserves downstream MCP results.
