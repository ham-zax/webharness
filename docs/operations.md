...
For the Personal Workstation Local domain, the outer 1MCP exposes only Local `tool_list`, `tool_schema`, `tool_call`, `fallback_dispatch`, and `tool_batch` under `tag:local`; the Local provider starts an inner 1MCP over stdio in normal direct mode. Unscoped `tool_list` reports only public logical servers with availability/tool counts; fallback-only Dev/Terminal mirrors are intentionally hidden from discovery, schema lookup, ordinary `tool_call`, and `tool_batch`. `fallback_dispatch` can target those mirrors when the normal writable MCP call is unavailable or unreliable, while preserving the same downstream arguments and result. Its `readOnlyHint` is intentional transport metadata, not a side-effect guarantee. `tool_batch` repeats one selected public downstream route over several structured argument objects with bounded concurrency. The inner config also contains `browser-devtools`, `browser-fast`, and optional owner-configured downstream MCPs from `MCP_LOCAL_SERVERS_FILE`. Owner stdio MCPs are supervised with `restartOnExit: true` by default. The Local child enables only 1MCP's reload management action for broker-owned targeted recovery, while the reserved `1mcp` namespace remains filtered and unreachable through model-facing Local routing. A failed direct call is never auto-replayed after recovery because its original outcome may be ambiguous. The renderer writes the inner config before the outer config and carries a deterministic inner-config revision so downstream changes recycle only Local. Stock lazy `tool_invoke` remains outside this path because direct mode preserves downstream MCP results.
